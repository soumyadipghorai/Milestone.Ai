...
async def analyze_student_report(user_id: str = Form(...), checklist_id: int = Form(...), file: UploadFile = File(...)): 
    if file.content_type != "application/pdf":
        raise HTTPException(status_code=400, detail="Invalid file type. Only PDF files are allowed.")
    
    unique_id = str(uuid.uuid4()) 
    analyzer_obj = PdfAnalyzer(
        unique_id= unique_id, pdf_file= file, student_id = user_id, 
